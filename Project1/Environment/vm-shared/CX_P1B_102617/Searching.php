...
<?php
function queryDB($query, $table)
{
  $db_connection = mysql_connect("localhost", "cs143", "");
  if(!$db_connection) 
  {
    $errmsg = mysql_error($db_connection);
    print "<html><body><div style=\"position:relative;left:100px;top:300px\">Connection failed:<br> $errmsg </br></div></body></html>";
    #Not yet
    exit(1);
  }
  mysql_select_db("CS143", $db_connection);
  #print "<html><body><div style=\"position:relative;left:100px;top:300px\"><br> $query </br></div></body></html>";
  $rs = mysql_query($query, $db_connection);
#foreach($names as $n)
#{
#  echo "<html><body><div style=\"position:relative;left:100px;top:300px\">@$n@</br></div></body></html>"; 
#}
  ...
  {if (mysql_num_fields($rs) != 0) //not update
    {
      #$affected = mysql_affected_rows($db_connection);
        ...
        //print "<br><table border = 1 width = 900><tr>";
        //print "<br><table style=\"border-radius:5px;border:1px solid #D0D0D0; width:900px; border-collapse: collapse;\"><tr>";
        print "<br><style>
...
{   
  $names = split(" ", $query);
  #echo "<html><body><div style=\"position:relative;left:200px;top:300px\">@$names[1]@</br></div></body></html>";
  $actorFirstQuery = "select concat(first, ' ',last) as name, dob as \"date-of-birth\" from Actor where upper(first) like '%$names[0]%' and upper(last) like '%$names[1]%'";
  $actorLastQuery = "select concat(first, ' ', last) as name, dob as \"date-of-birth\" from Actor where upper(first) like '%$names[1]%' and upper(last) like '%$names[0]%'";
  $unionQuery = "($actorFirstQuery) UNION ($actorLastQuery);";
  
  queryDB($unionQuery, "Actors");

  $movieQuery = "select title, year from Movie where upper(title) like '%$names[0]%_%$names[1]%' or upper(title) like '%$names[1]%_%$names[0]%';";
  print "<html><body><br></br></body></html>";
