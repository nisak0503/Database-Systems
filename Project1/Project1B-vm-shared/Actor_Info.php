...
<FORM METHOD = "GET" ACTION = "Show_ActorInfo.php">
 
  <h1>
  <?php echo "Actor Information Page:"?>
  </h1>
  <div style="margin:0;padding:0; width:900px;height:1px;background-color:#D0D0D0;overflow:hidden;"> </div>
  <br>
  <div><b><font size = "5">
  <?php echo "Actor Information is:"?>
  </font>
  </b>
  </div>
  ...
  {if (mysql_num_fields($rs) != 0) //not update
    {
        ...
        for ($i=0; $i<$num_fields; $i++)  //do not count actor id
        {
...
<?php
    ...
    $query = "SELECT CONCAT(first, ' ', last) as Name, sex, dob as \"Date of Birth\", dod as \"Date of Death\" from Actor where id = $id;";
    queryDB($query);
    print "<br>";
?>
...
<div style="margin:0;padding:0; width:900px;height:1px;background-color:#D0D0D0;overflow:hidden;"> </div>
  <br>
  <div><b><font size = "5">
  <?php echo "Actor's Movies and Role:"?>
  </font>
  </b>
  </div>
...
<?php
    ...
    $query = "SELECT ma.role as Role, m.title as \"Movie Title\", m.id as id from Actor a, MovieActor ma, Movie m where a.id = $id and a.id = ma.aid and ma.mid = m.id;";
    queryDBMovie($query);
    print "<br>";
?>
<br>
