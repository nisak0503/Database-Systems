...
<?php
function queryDB($query, $table, $isActor)
{
  ...
  {if (mysql_num_fields($rs) != 0) //not update
    {
      #$affected = mysql_affected_rows($db_connection);
        ...
        for ($i=0; $i<$num_fields-1; $i++)  //not print final id
        {
        ...
        while($row = mysql_fetch_row($rs))
        {
          ...
          foreach($row as $unit)
          {
            ...
            if (!$unit)
              ...
              {
                print "<a href=\"";
                if($isActor) print "Actor_Info.php?identifier=";
                else print "Movie_Info.php?identifier=";
...
{   
  ...
  $isActor = True;
  queryDB($unionQuery, "Actors", $isActor);

  $movieQuery = "select title, year, id from Movie where upper(title) like '%$names[0]%_%$names[1]%' or upper(title) like '%$names[1]%_%$names[0]%' or upper(title) like '%$names[0]%';";
  print "<html><body><br></br></body></html>";
  $isActor = False;
  queryDB($movieQuery, "Movies", $isActor);
 } 
